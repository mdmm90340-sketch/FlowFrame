...
本工作副本为 **1.2.0 本地测试版**，暂不公开发布；GitHub 已发布版本仍为 1.1.2。手机 APK 提供 `arm64-v8a`，支持目标为 Android 7.0 及以上。普通 4 KiB 虚拟机与实验性 16 KiB x86_64 镜像的原生媒体流程通过，但其余上游组件仍有 RELRO 对齐缺口，尚不能承诺全面兼容 16 KiB 设备。平台网络与真实设备的验收范围见 [测试报告](docs/VERIFICATION-1.2.0.md)。
