...
本工作副本为 **1.2.1 本地效率优化版**，暂不公开发布；GitHub 已发布版本仍为 1.1.2。手机 APK 提供 `arm64-v8a`，支持目标为 Android 7.0 及以上。此轮减少队列轮询、无效历史写入和界面重复转换，结果见 [优化验收报告](docs/OPTIMIZATION-1.2.1.md)。此前的平台与设备范围保留在 [1.2.0 测试报告](docs/VERIFICATION-1.2.0.md)；其余原生组件仍有 RELRO 对齐缺口，尚不能承诺全面兼容 16 KiB 设备。
